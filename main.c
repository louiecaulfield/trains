...
//--------------- SEND request & get results URI
	check(curl_http_init()==0,"Failed to initialise curl");

	res = fetch_html_post("http://be.voyages-sncf.com/vsc/train-ticket/?_LANG=en",
		postfields, &tdoc);
...
//--------------- RESULT ------------------
	...
	res = fetch_html_get(tidyAttrValue(findAttribute(link, "href")), &tdoc);
